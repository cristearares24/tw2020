...
function myFunction(elem)
{
    ...
    inProgress = true;
    localStorage.setItem('inProgress2', JSON.stringify(inProgress));
    let btn = document.getElementsByClassName('btn');
    let i;
    localStorage.setItem('btn2'+elem.id, JSON.stringify(true));
    ...
    localStorage.setItem('allRead2', JSON.stringify(inProgress));
}
...
window.onload = function(){
    ...
    {
        let btn = JSON.parse(localStorage.getItem('btn2' + prog[j].id));
        if(btn)
        {
