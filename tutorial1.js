...
function myFunction(elem)
{
    ...
    inProgress = true;
    localStorage.setItem('inProgress1', JSON.stringify(inProgress));
    let btn = document.getElementsByClassName('btn');
    let i;
    localStorage.setItem('btn1'+elem.id, JSON.stringify(true));
    ...
    localStorage.setItem('allRead1', JSON.stringify(inProgress));
}
...
window.onload = function(){
    ...
    {
        let btn = JSON.parse(localStorage.getItem('btn1' + prog[j].id));
        if(btn)
        {
