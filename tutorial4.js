...
function myFunction(elem)
{
    ...
    inProgress = true;
    localStorage.setItem('inProgress4', JSON.stringify(inProgress));
    let btn = document.getElementsByClassName('btn');
    let i;
    localStorage.setItem('btn4'+elem.id, JSON.stringify(true));
    ...
    localStorage.setItem('allRead4', JSON.stringify(inProgress));
}
...
window.onload = function(){
    ...
    {
        let btn = JSON.parse(localStorage.getItem('btn4' + prog[j].id));
        if(btn)
        {
